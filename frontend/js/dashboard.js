...
async function fetchUnifiedData(force = false) {
    ...
    try {
        const res = await fetch('https://qrsystem-icqn.onrender.com/api/admin/analytics');
...
document.getElementById('createEventForm').addEventListener('submit', async (e) => {
    ...
    try {
        const res = await fetch('https://qrsystem-icqn.onrender.com/api/admin/create-card', { 
            method: 'POST', 
            body: formData 
        });
        if(res.ok) {
...
async function deleteEvent(id) {
    ...
    try {
        await fetch(`https://qrsystem-icqn.onrender.com/api/admin/event/${id}`, { method: 'DELETE' });
...
function triggerImageUpdate(adminId) {
    ...
    input.onchange = async (e) => {
        ...
        try {
            const res = await fetch(`https://qrsystem-icqn.onrender.com/api/admin/event/${adminId}/image`, {
                method: 'PUT',
                body: formData
            });
            const data = await res.json();
...
document.getElementById('uploadForm').addEventListener('submit', async (e) => {
    ...
    try {
        const res = await fetch(`https://qrsystem-icqn.onrender.com/api/events/upload/${adminId}`, {
            method: 'POST',
            body: formData
        });
        const data = await res.json();
...
async function loadRegistrations() {
    ...
    try {
        const res = await fetch(`https://qrsystem-icqn.onrender.com/api/admin/dashboard/${targetAdminId}`);
        if (res.status === 404) {
            window.currentTargetAdminId = null;
...
document.getElementById('verifyIdForm').addEventListener('submit', async (e) => {
    ...
    try {
        const res = await fetch('https://qrsystem-icqn.onrender.com/api/admin/verify', {
            method: 'POST',
...
async function loadNotifications() {
    try {
        const res = await fetch('https://qrsystem-icqn.onrender.com/api/admin/notifications');
        const data = await res.json();
...
async function markAllNotificationsRead() {
    try {
        await fetch('https://qrsystem-icqn.onrender.com/api/admin/notifications/read-all', { method: 'POST' });
        loadNotifications();
    } catch(e) {}
}
...
async function handleManualEntry(e) {
    ...
    try {
        const res = await fetch(`https://qrsystem-icqn.onrender.com/api/admin/event/${adminId}/manual-entry`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function bulkDeleteEventRecords() {
    ...
    try {
        const res = await fetch(`https://qrsystem-icqn.onrender.com/api/admin/event/${adminId}/records`, {
            method: 'DELETE'
        });
        const data = await res.json();
...
async function deleteSingleRegistration(scholarId) {
    ...
    try {
        const res = await fetch(`https://qrsystem-icqn.onrender.com/api/admin/event/${adminId}/record/${scholarId}`, { method: 'DELETE' });
        const data = await res.json();
        if(res.ok) {
            loadRegistrations();
            loadEvents();
